...
function ProductList() {
  ...
  const submitUpdate = async (id) =>{
    console.log(id);
    try {
      const response = await axios.put(`http://localhost:5000/update/${id}`,updateProduct);
...
<div className="modal fade" id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
 ...
 style={{width:'200px'}} onClick={()=>{submitUpdate(updateProduct._id)}} type="submit" >Edit Product</button>
    </div>
          </div>
      </div>
